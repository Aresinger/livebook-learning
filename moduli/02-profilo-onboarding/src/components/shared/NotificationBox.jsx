...
export default function NotificationBox({confirmation, setConfirmation,setView}) {
function handleClick (){
    setConfirmation({active:false,message: ''});
    setView("login")
  ...
  return (
    <>
      ...
      <div className='border border-white/15 h-36 justify-self-center  fixed top-28 w-80 m-10 rounded-lg bg-violet-900 bg-opacity-50 z-50 shadow-2xl'>
           ...
           <p className='text-white text-sm m-5 font-medium'  >{confirmation?.message}</p>
    
        <div className='justify-self-center'>
          <button className='btn-primary ' onClick={() =>handleClick()}>Continua</button>
        </div>
      </div>
    </>
  )
